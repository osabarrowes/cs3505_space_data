...
// this reads in all of the files in the given directory, then sorts them
// this is necessary because unlike linux and windows, AWS does NOT automatically
//  sort the files inside of the efs because it's a very special boi
void populate_vector(std::vector<std::string> &vector, std::string filepath)
{
    std::filesystem::directory_iterator iter(filepath);
    while(iter != std::filesystem::end(iter))
    {
        auto p = *iter;
        vector.push_back(p.path());
        iter++;
    }
...
// this advances the vector index up to the given time in the directory
void advance_index(int &idx, std::vector<std::string> vector, int time)
{
    // while the filename number of the next item is less than the starting time
    // and there's still room to advance
    while(idx + 1 < vector.size() && std::stoi(extract_filename(vector[idx + 1])) < time)
    {
        idx++; 
    }
}
...
int main(int argc, char **argv) 
{
    ...
    // make and initialize the vectors
    std::vector<std::string> isscoords_vector;
    std::vector<std::string> issfeed_vector;
    std::vector<std::string> lightningmap_vector;
    std::vector<std::string> supernovae_vector;
    std::vector<std::string> twitch_vector;
    std::vector<std::string> weather_vector;

    populate_vector(isscoords_vector, "framesegments/isscoords");
    populate_vector(issfeed_vector, "framesegments/issfeed");
    populate_vector(lightningmap_vector, "framesegments/lightningmap");
    populate_vector(supernovae_vector, "framesegments/supernovae");
    populate_vector(twitch_vector, "framesegments/twitch");
    populate_vector(weather_vector, "framesegments/weather");

    // make and initialize the vector indices
    int isscoords_idx =    0;
    int issfeed_idx =      0;
    int lightningmap_idx = 0;
    int supernovae_idx =   0;
    int twitch_idx =       0;
    int weather_idx =      0;
    advance_index(isscoords_idx,    isscoords_vector,    start_time);
    advance_index(issfeed_idx,      issfeed_vector,      start_time);
    advance_index(lightningmap_idx, lightningmap_vector, start_time);
    advance_index(supernovae_idx,   supernovae_vector,   start_time);
    advance_index(twitch_idx,       twitch_vector,       start_time);
    advance_index(weather_idx,      weather_vector,      start_time);
    ...
    for(int i = 0; i < 600; i++)
    {
        ...
        // advance all indices to the new point in time
        advance_index(isscoords_idx,    isscoords_vector,    scaled_time);
        advance_index(issfeed_idx,      issfeed_vector,      scaled_time);
        advance_index(lightningmap_idx, lightningmap_vector, scaled_time);
        advance_index(supernovae_idx,   supernovae_vector,   scaled_time);
        advance_index(twitch_idx,       twitch_vector,       scaled_time);
        advance_index(weather_idx,      weather_vector,      scaled_time);

        // load the frame segments
        AVFrame *bg_frame           = load_frame("framesegments/bg.png");
        AVFrame *isscoords_frame    = load_frame(isscoords_vector[isscoords_idx].c_str());
        AVFrame *issfeed_frame      = load_frame(issfeed_vector[issfeed_idx].c_str());
        AVFrame *lightningmap_frame = load_frame(lightningmap_vector[lightningmap_idx].c_str());
        AVFrame *supernovae_frame   = load_frame(supernovae_vector[supernovae_idx].c_str());
        AVFrame *twitch_frame       = load_frame(twitch_vector[twitch_idx].c_str());
        AVFrame *weather_frame      = load_frame(weather_vector[weather_idx].c_str());
